...
// =======================
// CONTENIDO DE FAVORITOS VACÍOS
// =======================
function construirContenidoFavoritos() {
  ...
  heartIcon.style.width = "80px";
  heartIcon.style.height = "80px";
  heartIcon.style.color = "#ccc";
  heartIcon.style.marginBottom = "20px";

  const title = document.createElement("h2");
  title.textContent = "Tus favoritos están vacíos";
  title.className = "cart-empty-title";

  const message = document.createElement("p");
  message.textContent = "Guarda tus productos favoritos para verlos aquí";
...
function agregarEventosHeader() {
  // Botón de perfil (usuario) -> redirige a login.html
  ...
  // Botón de carrito -> redirige a cart.html
  ...
  // El botón de corazón ya está en la página de favoritos, pero si quieres que también redirija a la misma página (o a index), puedes dejarlo.
  // Por ahora no se modifica.
}

// =======================
// PÁGINA COMPLETA
// =======================
async function construirPaginaFavoritos() {
  if (!body) return;
  body.innerHTML = "";
  const header = construirHeader();
  const main = construirContenidoFavoritos();
  ...
  // Añadir eventos a los iconos del header
